...
export default function PerpetualDashboard() {
    ...
    return (
        <div className="space-y-4">
            <h1 className="text-2xl font-bold">Perpetual License / Software MA — {year}</h1>
            <div className="card overflow-x-auto">
                <table className="table-clean">
                    <thead>
                        <tr>
                            <th>Code</th><th>Description</th><th>Customer</th><th>Status</th>
                            <th>License/MA Name</th><th>Type</th><th>Start</th><th>End</th>
                            <th className="text-right">Revenue</th><th className="text-right">Cost</th>
                            <th className="text-right">GM</th><th className="text-right">% Rec</th>
                            <th className="text-right">Rec. Revenue</th><th className="text-right">Rec. GM</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading && <tr><td colSpan={14} className="text-center py-6 text-slate-400">Loading...</td></tr>}
                        {!loading && rows.length === 0 && <tr><td colSpan={14} className="text-center py-6 text-slate-400">No data</td></tr>}
                        {rows.map(r => (
                            <tr key={r.id}>
                        ...
                        {!loading && rows.length > 0 && (
                            <tr className="bg-slate-50 font-semibold">
                                <td colSpan={12} className="text-right">Totals</td>
